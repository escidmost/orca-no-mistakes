...
Reviewers run as fresh workers in disposable child worktrees. A compatible fixer terminal and child worktree are retained across fix rounds, with each committed repair applied back to the coordinator's isolated gate worktree; changing the primary configured fixer, observing that its worktree no longer matches the pre-round head, finding uncommitted changes, or failing to acknowledge retained-worker completion releases it before launching a fresh session. Before dispatching another round, the coordinator verifies that the expected agent still owns the retained terminal; a terminal that returned to its shell is released and retried through the fresh configured chain. If a retained terminal fails before accepting another round, including while receiving its next authenticated preamble, the coordinator releases it and retries the full configured chain in fresh child worktrees. Retained fixers are released before a human gate opens. Every fixer round must leave a clean worktree and create a new commit.
...
- `cursor` launches through native `orca orchestration worker-start`, which receives the resolved model, effort, and timeout as flags. Fresh Claude, Codex, and Antigravity workers create their authenticated dispatch first, then launch through a local terminal with a short instruction to read the mode-0600 preamble artifact; retained sessions receive the same file instruction through Orca's agent-prompt terminal transport. Codex reasoning effort maps to `model_reasoning_effort` and automation uses `--dangerously-bypass-approvals-and-sandbox`. This avoids native prompt-injection stalls, empty initial TUIs, prompt-processing races, and OS argument-size limits.

...
- `codex` and `cursor` launch through native `orca orchestration worker-start`, which receives the resolved model, effort, and timeout as flags. Claude launches in a terminal, then receives the authenticated dispatch preamble through terminal input after readiness is detected; this avoids treating Claude's immediate processing of a large prompt as a failed injection.

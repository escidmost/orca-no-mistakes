...
- `codex` and `cursor` launch through native `orca orchestration worker-start`, which receives the resolved model, effort, and timeout as flags. A fresh Claude worker creates its authenticated dispatch first, then launches with the protected preamble file as its positional CLI prompt; retained Claude sessions receive later prompts through Orca's agent-prompt terminal transport. This avoids both an empty initial TUI and treating Claude's immediate prompt processing as a failed injection.

...
After successful execution, anchor the terminal commit at `refs/no-mistakes/recover/<run-id>` before custody transfer. A clean initiating checkout still at the submitted commit can advance. A dirty or diverged checkout remains intact, with recovery instructions returned to the operator. The diagram places this outcome in the recovery lane only for layout; it remains a normal successful custody-return branch, not a crash path.

...
- Fixer presentation identity is the originating reviewer analysis plus a retry ordinal. Internal evidence rounds remain unchanged, while user-facing labels progress from `Review fix 3` to `Review fix 3 retry 1` until a new reviewer analysis begins.
...
- The in-process TUI owns terminal mode only after compatibility checks succeed. It enters raw-input and alternate-screen modes, then restores input mode, cursor, and screen exactly once before normal exit, handled renderer fallback, errors, orderly Cancel completion, or immediate Force stop; it also restores before `SIGTSTP` and re-enters and redraws after `SIGCONT`. Tab and Shift-Tab move between regions, arrows move within a region, Enter acts or opens, Escape returns, `G` reopens an open decision gate from the rail, inside a finding gate arrows select a finding, `F` selects Fix and `A` selects Approve as is for its ID, Page Up/Down scroll descriptions, and Enter reviews the completed choices before an explicit second Enter confirms the canonical resolution; other gates retain their offered choices while Escape returns to the rail leaving the gate unanswered and restoring the pinned stage and focus state, `A` toggles the local gate auto-responder and synchronizes durable Auto-fix mode to the resulting state from the rail and from gates without the finding editor, `R` — offered only when the error panel shows a resumable error — starts the next attempt of the same run, `C` opens Cancel confirmation, the first Ctrl-C requests orderly Cancel, a second Ctrl-C Force stops, and raw Ctrl-Z suspends the TUI through that same SIGTSTP/SIGCONT restore-and-re-enter contract.

...
- The in-process TUI owns terminal mode only after compatibility checks succeed. It enters raw-input and alternate-screen modes, then restores input mode, cursor, and screen exactly once before normal exit, handled renderer fallback, errors, orderly Cancel completion, or immediate Force stop; it also restores before `SIGTSTP` and re-enters and redraws after `SIGCONT`. Tab and Shift-Tab move between regions, arrows move within a region, Enter acts or opens, Escape returns, `C` opens Cancel confirmation, the first Ctrl-C requests orderly Cancel, and a second Ctrl-C Force stops.

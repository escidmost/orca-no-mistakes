...
- **Tri-Modal Launch Dispatch Strategy**:
  ...
  2. *Terminal Spawn + Authenticated Dispatch*: For CLI harnesses (`claude`, `codex`, `opencode`, `grok`, `gemini`), the adapter spawns an isolated terminal, sends the agent startup command formatted with configured model, effort/variant, and `agent_args_override`, and awaits readiness. Fresh terminals wait 12 seconds when the controller's default shell is fish so hidden panes can pass fish's ten-second terminal-query timeout; `WORKER_SHELL_STARTUP_DELAY_MS` overrides this compatibility delay. OpenCode, Grok, and Gemini use injected dispatch. Fresh Claude, Codex, and Antigravity workers create the authenticated dispatch without injection, store its preamble in a protected artifact, and launch with a short instruction to read that file; retained sessions receive the same file instruction through Orca's agent-prompt terminal transport. Codex uses `model_reasoning_effort` plus `--dangerously-bypass-approvals-and-sandbox`. This keeps preambles out of native prompt-injection stalls and argv size limits.

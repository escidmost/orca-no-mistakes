...
`abandon` explicitly ends a failed or orphaned run's resumability while retaining its evidence, attempt outcomes, artifacts, and Git refs. Run it on the machine that ran the coordinator: it requires the latest attempt coordinator PID to be absent, or for a run interrupted before its first attempt, the initial coordinator PID recorded atomically at run creation. It rejects pending resume claims and any lease without a matching attempt generation, records the reason, marks the run cancelled, and releases only its lease. Older records without verifiable coordinator ownership remain fail-closed. It also releases the run's bound submission admission, allowing the identical branch, candidate, and intent to start a fresh run. It does not remove worktrees or stop workers; use stranded recovery for those resources. This also removes that run's publication-route dependency. The route blocker counts retained in-progress and resumable failed records, not live processes.

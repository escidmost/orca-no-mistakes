...
Decision gates offer `approve`, `fix [ids][: guidance]`, `skip`, or `stop`; unknown resolutions fail closed. With `--tui`, use Up/Down to choose, Enter to select and confirm, Esc to return unanswered, and `G` to reopen an open gate. Pending gates can also be resolved through Orca's native gate interface; the bundled [`/orca-no-mistakes` skill](skills/orca-no-mistakes/SKILL.md) contains the commands.

...
Decision gates offer `approve`, `fix [ids][: guidance]`, `skip`, or `stop`; unknown resolutions fail closed. With `--tui`, use Up/Down to choose, Enter to select and confirm, Esc to return unanswered, `G` to reopen an open gate, and `A` to toggle the durable Auto-fix mode; when the error panel reports a resumable stage failure, `R` starts the next attempt of the same run from its durable checkpoint (offered only after the failed attempt is durably settled and its workers cleaned) while `C` leaves the run stopped; `C` opens the Cancel confirmation (Enter confirms Cancel, Esc keeps the run active), the first Ctrl-C requests an orderly Cancel, and a second Ctrl-C Force stops, leaving marker-based recovery that `prune --stranded` later settles as cancelled. Pending gates can also be resolved through Orca's native gate interface; the bundled [`/orca-no-mistakes` skill](skills/orca-no-mistakes/SKILL.md) contains the commands.

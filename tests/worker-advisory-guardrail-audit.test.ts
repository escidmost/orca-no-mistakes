import assert from "node:assert/strict";
import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";
import test from "node:test";

import { DomainLedger } from "../scripts/ledger.ts";
import {
  runPipeline,
  FixerPolicyViolationError,
  type GitOperations,
  type OrcaOperations,
  type StageReport,
} from "../scripts/orca-no-mistakes.ts";

for (const [mode, weakened, recovery] of [
  ["advisory", false, "approve"],
  ["advisory", true, "approve"],
  ["strict", false, "approve"],
  ["strict", true, "approve"],
  ["strict", false, "fix"],
  ["strict", false, "stop"],
] as const) {
  test(`${mode} content recovery=${recovery} (weakened=${weakened})`, async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "orca-advisory-audit-"));
    const previousHome = process.env.ORCA_NO_MISTAKES_HOME;
    process.env.ORCA_NO_MISTAKES_HOME = temp;
    const ledger = new DomainLedger(":memory:");
    const runId = "advisory-audit-run";
    const base = "b".repeat(40);
    const workerHead = "c".repeat(40);
    let head = "a".repeat(40);
    let task = 0;
    let dispatch = 0;
    let gates = 0;
    const pass = (summary: string): StageReport => ({ findings: [], summary });
    const reviewReports: StageReport[] = [
      {
        findings: [
          {
            action: "auto-fix",
            description: "Repair implementation.",
            id: "review-finding",
            severity: "error",
          },
        ],
        summary: "review finding",
      },
      pass("fix committed"),
      weakened ? {
        findings: [{ id: "unexplained-policy-relaxation", action: "ask-user", severity: "error", description: "Existing assertion was weakened." }],
        summary: "assertion weakened",
      } : pass("clean review"),
    ];
    const violation =
      "fixer modified pre-existing test files: tests/existing.test.ts";
    const orca: OrcaOperations = {
      async createRun() {
        return runId;
      },
      async createTask() {
        return `task-${++task}`;
      },
      async startWorker(taskId, launch) {
        const id = `dispatch-${++dispatch}`;
        const report =
          launch.stage === "review"
            ? (reviewReports.shift() ?? pass("review"))
            : pass(launch.stage);
        return {
          dispatchId: id,
          report,
          taskId,
          worktreePath:
            launch.worktree === "new-child" ? path.join(temp, id) : undefined,
        };
      },
      async finishWorker() {},
      async removeWorktree() {},
      async completeTask() {},
      async createGate(_task, question) {
        if (mode === "strict" && gates === 0) {
          assert.match(question, new RegExp(workerHead));
          assert.match(question, /Approval does not waive reviewer findings/);
          assert.equal(head, "a".repeat(40));
        } else if (recovery === "fix") {
          assert.match(question, /fixer-policy-violation/);
          assert.equal(head, "a".repeat(40));
        } else {
          assert.match(question, /unexplained-policy-relaxation/);
          assert.equal(head, workerHead);
        }
        gates += 1;
        return `gate-${gates}`;
      },
      async waitForGate() {
        // Human recovery waits must outlive the configured worker deadline.
        if (mode === "strict" && gates === 1) await new Promise((resolve) => setTimeout(resolve, 1100));
        return gates === 1 ? recovery : recovery === "fix" ? "stop" : "approve";
      },
      async setWorktreeStatus() {},
    };
    const git: GitOperations = {
      async assertReady() {
        return { base: "main", baseOid: base, branch: "feature", head, root: temp };
      },
      async assertClean() {},
      async assertFixerChangesAllowed(_path, _before, _after, guardrails) {
        assert.equal(guardrails, mode);
        if (guardrails === "strict") throw new FixerPolicyViolationError(violation, [violation]);
        return { changed: true, guardrailViolations: [violation] };
      },
      async head() {
        return head;
      },
      async diffBase() {
        return "";
      },
      async rebase() {
        return { ...pass("rebased"), rebaseUpstreamHead: base };
      },
      async resolveRefSha() {
        return base;
      },
      async showFile() {
        return `auto_fix:\n  allow_review_autofix: true\n${mode === "strict" ? "  guardrails: strict\n" : ""}defaults:\n  timeout_ms: 1000\n`;
      },
      async pathExists() {
        return true;
      },
      async policySha256() {
        return "d".repeat(64);
      },
      async resolveBaseOid() {
        return base;
      },
      async applyWorktreeCommits() {
        head = workerHead;
        return true;
      },
      async headOf() {
        return workerHead;
      },
      async worktreeIsReusable() {
        return false;
      },
      async anchorRecoveryRef() {},
    };

    try {
      const run = runPipeline({ intent: "Record advisory guardrail changes." }, orca, git, ledger);
      if (recovery !== "approve") {
        await assert.rejects(run, /stopped/i);
        assert.equal(head, "a".repeat(40));
        assert.equal(gates, recovery === "fix" ? 2 : 1);
        return;
      }
      await run;

      assert.equal(gates, Number(mode === "strict") + Number(weakened));
      assert.equal(head, workerHead);
      assert.equal(reviewReports.length, 0);
      const allAudits = ledger.listGateAudit(runId);
      if (mode === "strict") {
        const recoveryAudit = allAudits.find((audit) => audit.gate_kind === "guardrail" && audit.decision === "advisory" && audit.question.includes("Fixer candidate"));
        assert.ok(recoveryAudit);
        assert.match(recoveryAudit.question, new RegExp(workerHead));
        const accepted = ledger.listPresentationSnapshots(runId).find((snapshot) =>
          snapshot.transition.kind === "gate-resolved" && snapshot.transition.decision === "advisory",
        );
        assert.equal(accepted?.stages.find((stage) => stage.id === "review")?.approvedFindings, 0);
      }
      const audits = allAudits.filter((audit) => audit.decision === "advisory" && !audit.question.includes("Fixer candidate"));
      assert.equal(audits.length, 1);
      assert.equal(audits[0].gate_kind, "guardrail");
      assert.equal(audits[0].decision, "advisory");
      assert.match(audits[0].question, new RegExp(`^\\[guardrails: ${mode}\\]`));
      assert.ok(audits[0].resolved_at);
      assert.deepEqual(JSON.parse(audits[0].resolution), [violation]);
    } finally {
      ledger.close();
      if (previousHome === undefined) delete process.env.ORCA_NO_MISTAKES_HOME;
      else process.env.ORCA_NO_MISTAKES_HOME = previousHome;
      await rm(temp, { recursive: true, force: true });
    }
  });

}

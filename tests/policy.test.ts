...
test("effectivePolicyHash is stable across key order and changes with content", () => {
  const left = effectivePolicyHash({
    defaults: { model: "m" },
    stages: { review: {} },
  });
  const right = effectivePolicyHash({
    stages: { review: {} },
    defaults: { model: "m" },
  });
  ...
  assert.notEqual(
    left,
    effectivePolicyHash({ defaults: { model: "n" }, stages: { review: {} } }),
  );
});
...
test("extractTrustedBaseConfig reads .orca/no-mistakes.yaml from the base ref", async () => {
  const yaml = "stages:\n  review:\n    reviewer:\n      agent: claude\n";
  const git = fakeGit(
    { "origin/main:.orca/no-mistakes.yaml": yaml },
    { "origin/main": "basesha" },
  );
  const { config, provenance } = await extractTrustedBaseConfig(git, "main");
  assert.deepEqual(config, {
    stages: { review: { reviewer: { agent: "claude" } } },
  });
...
test("an unresolvable trusted base ref fails closed instead of certifying a local ref", async () => {
  ...
  assert.deepEqual(
    git.requested.filter((entry) => entry.startsWith("show:")),
    [],
  );
});
...
test("invalid YAML on the trusted base fails closed", async () => {
  const git = fakeGit({
    "origin/main:.orca/no-mistakes.yaml": "defaults: [unclosed\n",
  });
  await assert.rejects(
    extractTrustedBaseConfig(git, "main"),
    /YAML parse error|Invalid configuration/,
  );
});
...
test("resolveRunPolicy flags local bypasses and prefers explicit --config paths", async () => {
  ...
  try {
    ...
    assert.ok(
      !git.requested.some((call) => call.startsWith("sha:")),
      "bypass never touches git",
    );
  } finally {
...
test("a failed absence inspection fails closed instead of certifying an empty policy", async () => {
  const git = fakeGit({});
  git.pathExists = async () => {
    throw new Error(
      "could not inspect origin/main:.orca/no-mistakes.yaml: object fetch failed",
    );
  };
  await assert.rejects(
    extractTrustedBaseConfig(git, "main"),

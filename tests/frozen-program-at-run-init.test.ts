...
test("the frozen report command exists before the first worker launches", async () => {
  ...
  let launches = 0;
  const startWorker = orca.startWorker.bind(orca);
  orca.startWorker = async (taskId, launch) => {
    launches += 1;
    frozenAtLaunch.push(
      ...readdirSync(artifactsDir).filter((entry) => /^program-[0-9a-f]{8}$/.test(entry)),
    );
    return startWorker(taskId, launch);
  };
  try {
    await runPipeline({ intent: "Freeze the program at run start" }, orca, git);
    assert.ok(launches > 0);
    assert.equal(new Set(frozenAtLaunch).size, 1);
    assert.equal(frozenAtLaunch.length, launches);
    const program = path.join(artifactsDir, frozenAtLaunch[0]);
    for (const entry of ["bin/orca-no-mistakes", "scripts/orca-no-mistakes.ts", "package.json", "node_modules/yaml/package.json", "node_modules/zod/package.json"]) {

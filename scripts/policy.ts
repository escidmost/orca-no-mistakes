...
export async function extractTrustedBaseConfig(
  ...
  if (!sha) {
    throw new Error(`could not resolve trusted base ref ${ref}; fetch origin or pass --allow-local-config`)
  }
  const contents = await git.showFile(ref, POLICY_CONFIG_PATH)
  if (contents === undefined && (await git.pathExists(ref, POLICY_CONFIG_PATH))) {
...
export async function loadLocalPolicyConfig(sourcePath: string): Promise<OrcaNoMistakesConfig> {
  const contents = await readPolicyFile(sourcePath)
  if (contents === undefined) throw new Error(`--config could not read ${sourcePath}: no such file`)
...
export async function resolveRunPolicy(options: {
  ...
  if (options.configPath) {
    ...
    const contents = await readPolicyFile(path.join(path.resolve(options.repoRoot), POLICY_CONFIG_PATH))
    config = contents === undefined ? {} : parseConfigYaml(contents)
  }
  return { config, provenance: { effectivePolicyHash: effectivePolicyHash(config), localBypass: true } }
}
